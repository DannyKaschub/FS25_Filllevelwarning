...
if g_specializationManager:getSpecializationByName("fillLevelWarning") == nil then
  ...
  for vehicleTypeName, vehicleType in pairs(g_vehicleTypeManager.types) do
    ...
    if  SpecializationUtil.hasSpecialization(Combine, vehicleType.specializations) and
		SpecializationUtil.hasSpecialization(Drivable, vehicleType.specializations) and 
	not SpecializationUtil.hasSpecialization(Baler, vehicleType.specializations) then
